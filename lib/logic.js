...
async function openTrustAccount(tx) {
    ...
    const trustAccountRegistry = await getAssetRegistry(`${NS}.TrustAccount`);
    await trustAccountRegistry.add(trustAccount);

    const formRegistry = await getAssetRegistry(`${NS}.OpenTrustAccountNoticeForm`);
    await formRegistry.add(form);

    // Notifies the financial institution with event
    let event = factory.newEvent(NS, 'TrustAccountOpenedEvent');
    event.form = form;
    emit(event);
}
...
async function endorseTrustAccount(tx) {
    ...
    let event = factory.newEvent(NS, 'TrustAccountEndorsedEvent');
    event.form = form;
    emit(event);
}
...
async function approveTrustAccount(tx) {
    ...
    // Notifies the governance officer that this form is endorsed
    let event = factory.newEvent(NS, 'TrustAccountApprovedEvent');
    event.form = form;
    emit(event);
}
